      ...
      bcrypt.hash(password, 10, async function (err, hash) {
        if (err) {
          ...
          try {
            ...
            await client.query(
              `INSERT INTO users (user_email, password, user_id, first_name, last_name) VALUES ('${email}', '${hash}', '${postIdRandStr}', '${capFirstName}', '${capLastName}');`
            );
            client.release();
            res.status(200).json({
            ...
            console.log(
              `Error when adding user to database: '${error.detail}' - Full error: ${error}`
            );
            res.status(409).json({
...
router.post("/login", (req, res) => {
  try {
    async function login() {
      ...
      bcrypt
        .compare(req.body.password, databasePassword)
        .then(function (passesValidation) {
          if (passesValidation) {
...
router.put("/userDetails", authenticateToken, (req, res) => {
  async function updateUserDetails() {
    try {
      ...
      const client = await pool.connect();
      const databaseQuery = await client.query(
        `SELECT * FROM users WHERE user_id = '${decodedUserId}'`
      );
      client.release();
        ...
        bcrypt
          ...
          .then(function (passesValidation) {
            if (passesValidation) {
              ...
              const capLastName =
                req.body.lastName[0].toUpperCase() + req.body.lastName.substring(1);

              bcrypt.hash(req.body.newPassword, 10, async function (err, hash) {
                const client = await pool.connect();
                await client.query(
                  `UPDATE users SET password = '${hash}', first_name = '${capFirstName}', last_name = '${capLastName}' WHERE user_id = '${decodedUserId}';`
                );
                client.release();
                res.json({
  ...
  async function deleteUser() {
    try {
      ...
      if (databaseUserId === decodedUserId) {
        bcrypt
          .compare(req.body.password, databasePassword)
          .then(async function (passesValidation) {
            if (passesValidation) {
              const client = await pool.connect();
              const databaseQuery = await client.query(
                `SELECT file_upload FROM posts WHERE user_id = '${decodedUserId}';`
              );
              await client.query(
                `DELETE FROM posts WHERE user_id = '${decodedUserId}';`
              );
              await client.query(
                `DELETE FROM users WHERE user_id = '${decodedUserId}'`
              );
              client.release();
              res.json({
              ...
              databaseQuery.rows.forEach((row) =>
                fileUploadsArray.push(row.file_upload.split("/uploads/")[1])
              );

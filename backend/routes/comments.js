...
router.post("/addComment", authenticateToken, (req, res) => {
  async function addComment() {
    try {
      ...
      await client.query(
        `UPDATE posts SET public_comments = array_append(public_comments, '${objectAsString}') WHERE post_id = '${req.body.post_Id}';`
      );
      const databaseQuery = await client.query(
        `SELECT * FROM posts WHERE post_id = '${req.body.post_Id}';`
      );
      client.release();
...
router.delete("/deleteComment", authenticateToken, (req, res) => {
  async function deleteComment() {
    try {
      ...
      const client = await pool.connect();
      await client.query(
        `UPDATE posts SET public_comments = array_remove(public_comments, '${commentToDeleteString}');`
      );
      client.release();
